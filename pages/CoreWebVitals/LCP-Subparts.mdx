import snippet from '../../snippets/CoreWebVitals/LCP-Subparts.js?raw'
...
```mermaid
flowchart LR
    A([Navigation<br/>Start]) --> B["⏱️ TTFB<br/><small>Server Response</small>"]
    B --> C["⏳ Load Delay<br/><small>Resource Discovery</small>"]
    C --> D["📥 Load Time<br/><small>Download Resource</small>"]
    D --> E["🎨 Render Delay<br/><small>Paint Element</small>"]
    E --> F([✅ LCP<br/>Complete])
...
flowchart TD
    ...
    TTFB -->|Yes| TTFB_Fix["✓ Use CDN<br/>✓ Enable caching<br/>✓ Optimize server"]
    Delay -->|Yes| Delay_Fix["✓ Preload LCP image<br/>✓ Remove render-blocking<br/>✓ Inline critical CSS"]
    Load -->|Yes| Load_Fix["✓ Compress images<br/>✓ Use WebP/AVIF<br/>✓ Use responsive images"]
    Render -->|Yes| Render_Fix["✓ Defer non-critical JS<br/>✓ Avoid client-side rendering<br/>✓ Use fetchpriority=high"]
